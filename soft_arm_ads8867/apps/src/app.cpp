...
int App::AppInMsgProc(byte * msg, uint len)
{
	...
	if(m_opCode > OTA_Load_CFG_Begain && m_opCode < OTA_Load_CFG_End)
    {
        ret = app->m_pOTACfg->OTACfgConfig_Proc(m_opCode, msg, len);
		CHK_ERR(ret,ERR);
    }
...
// CA_YD_170 USE
//	pMqttMsg->TS1Msg_t[count]= 10*(m_pAdc_Ky->Get_Ec20ADC0Value()-500);//100*
//	pMqttMsg->TS2Msg_t[count]= 10*(m_pAdc_Ky->Get_Ec20ADC1Value()-500);
	
	pMqttMsg->TS1Msg_tLen = 1;
	pMqttMsg->TS2Msg_tLen = 1;
	return OK;
}
int App::get_ads8867_Value()
{
	int ret;
	
	ret = m_pAdc_Ky->Get_ADS8867Value(0);
	CHK_ERR(ret,ERR);
	
	pMqttMsg->S1_Len = (m_pAdc_Ky->spi_buf_len-2)/2;
	App_log("pMqttMsg->S1_Len = %d",pMqttMsg->S1_Len);
	
//if 250mv/mm/s  use pMqttMsg->S1[i-1] = 4*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
//if 100mv/g  use pMqttMsg->S1[i-1] = 10*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;	
//					250mv/mm/s use					//
	for (int i = 1; i <  m_pAdc_Ky->spi_buf_len-2; i++)
		{
			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
				pMqttMsg->S1[i-1] = 4*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
			}
			else{
				pMqttMsg->S1[i-1] = -(4*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
			}
		}
	
	ret = m_pAdc_Ky->Get_ADS8867Value(1);
	CHK_ERR(ret,ERR);
	
	pMqttMsg->TS1Msg_sLen=(m_pAdc_Ky->spi_buf_len-2)/2; 
//	App_log("pMqttMsg->TS1Msg_sLen = %d",pMqttMsg->TS1Msg_sLen);

	for (int i = 1; i <  m_pAdc_Ky->spi_buf_len-2; i++)
		{
			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
				pMqttMsg->TS1Msg_s[i-1] = 4*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
			}
			else{
				pMqttMsg->TS1Msg_s[i-1] = -(4*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
			}
		}

	ret = m_pAdc_Ky->Get_ADS8867Value(2);
	CHK_ERR(ret,ERR);
	
	pMqttMsg->S2_Len = (m_pAdc_Ky->spi_buf_len-2)/2;
//	App_log("pMqttMsg->S2_Len = %d",pMqttMsg->S2_Len);

	for (int i = 1; i <  m_pAdc_Ky->spi_buf_len-2; i++)
		{
			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
				pMqttMsg->S2[i-1] = 4*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
			}
			else{
				pMqttMsg->S2[i-1] = -(4*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
			}
		}

	ret = m_pAdc_Ky->Get_ADS8867Value(3);
	CHK_ERR(ret,ERR);
	
	pMqttMsg->TS2Msg_sLen=(m_pAdc_Ky->spi_buf_len-2)/2; 
//	App_log("pMqttMsg->TS2Msg_sLen = %d",pMqttMsg->TS2Msg_sLen);

	for (int i = 1; i <  m_pAdc_Ky->spi_buf_len-2; i++)
		{
			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
				pMqttMsg->TS2Msg_s[i-1] = 4*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
			}
			else{
				pMqttMsg->TS2Msg_s[i-1] = -(4*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
			}
		}
//		*************			 100mv/g use	************			//
//	for (int i = 1; i <  m_pAdc_Ky->spi_buf_len-2; i++)
//		{
//			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
//				pMqttMsg->S1[i-1] = 10*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
//			}
//			else{
//				pMqttMsg->S1[i-1] = -(10*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
//			}
//		}
//	
//	ret = m_pAdc_Ky->Get_ADS8867Value(1);
...
//		{
//			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
//				pMqttMsg->TS1Msg_s[i-1] = 10*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
//			}
//			else{
//				pMqttMsg->TS1Msg_s[i-1] = -(10*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
//			}
//		}
//
//	ret = m_pAdc_Ky->Get_ADS8867Value(2);
...
//		{
//			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
//				pMqttMsg->S2[i-1] = 10*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
//			}
//			else{
//				pMqttMsg->S2[i-1] = -(10*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
//			}
//		}
//
//	ret = m_pAdc_Ky->Get_ADS8867Value(3);
...
//		{
//			if(m_pAdc_Ky->spi_buf[i*2+1]<=127){
//				pMqttMsg->TS2Msg_s[i-1] = 10*2*100*(2*2.5*(m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2])))/65536;
//			}
//			else{
//				pMqttMsg->TS2Msg_s[i-1] = -(10*2*100*(2*2.5*(65535.0-m_pAdc_Ky->signExtend(&m_pAdc_Ky->spi_buf[i*2]))/65536));
//			}
//		}
...
void* App::AppProc(void* data)
{
    ...
    {
		pThis->get_adc_Value();		
		ret = pThis->get_ads8867_Value();
		if(ret == ERR)
		{
			App_log("get_ads8867_Value err !");
		}
		pThis->AlarmJudgment();
		//NOT NORMAL SEND
		if(pThis->pMqttMsg->STA.ALL_STA == true)
		{
			if(pThis->m_pAuthorization->m_pATRZT_T.State)
			{
				...
				ret = pThis->m_pMosquittoPub->MqttProc_send(pThis->pMqttMsg);
				if(ret != OK)
				APP_LOG(APP, LOG_DEBUG, "App alarm message send err ! .");	
			}
		}
		//NORMAL SEND
		...
		//封装完数据，进行发送mqtt处理,send the message before ,judge the authorization whether exceed the time limit.
			...
			if(pThis->m_pAuthorization->m_pATRZT_T.State)
			{
				...
				ret = pThis->m_pMosquittoPub->MqttProc_send(pThis->pMqttMsg);
				if(ret != OK)
				APP_LOG(APP, LOG_DEBUG, "App message send err ! .");	
			}
