...
static const uint8_t table_crc_lo[] = {
 ...
 /*********************************************************************
  * @fn 	 _modbus_set_slave
  *
  * @brief	 Define the slave ID of the remote device to talk in master mode or set the internal slave ID in slave mode
  *
  * @param	 ctx, slave
  *
  * @return  int
...
/*********************************************************************
* @fn	   _modbus_rtu_build_response_basis
*
* @brief   Builds a RTU response header
*
* @param   sft, rsp
*
* @return  int
...
#elif defined modbus_rs485_use

	ret = Ql_GPIO_SetLevel(PINNAME_GPIO3,PINLEVEL_LOW);
	if(ret == RES_OK)
	printf("set the m_RS485_RD IO success !\n");
	ret = Ql_GPIO_GetLevel(PINNAME_GPIO3);
	printf("m_RS485_RD = %d value is = %d\n",PINNAME_GPIO3,ret);
	usleep(2100);
	printf("come in the _modbus_rtu_send programe !\n");
	count = write(ctx->s, req, req_length);
	usleep(2100);
	ret = Ql_GPIO_SetLevel(PINNAME_GPIO3,PINLEVEL_HIGH);
	if(ret == RES_OK)
	printf("set the m_RS485_RD IO success !\n");
	ret = Ql_GPIO_GetLevel(PINNAME_GPIO3);
	printf("m_RS485_RD = %d value is = %d\n",PINNAME_GPIO3,ret);
...
/*********************************************************************
* @fn	   _modbus_rtu_check_integrity
*
* @brief   The check_crc16 function shall return the message length if the CRC is
...
/*  */
/*********************************************************************
* @fn	   _modbus_rtu_connect
*
* @brief   Sets up a serial port for RTU communications
*
* @param   ctx
*
* @return  int
...
* @brief   new
*
* @param   device, baud, parity, data_bit, stop_bit
*
* @return  modbus_t*
*
* @author  liuch
