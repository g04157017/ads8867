...
static byte *get_shm(key_t key, size_t size, int flag)
{
    int shm_id = shmget(key, size, flag);
    if (shm_id < 0)
        return NULL;
    byte *p = (byte*)shmat(shm_id, NULL, 0);
    if (p == (byte*)-1)
        return NULL;
    return p;
}

static int get_or_create_shm(key_t key, size_t size, byte **addr)
{
...
int queue_init(queue_t *queue, key_t shm_key, uint mem_size, char *file, size_t file_max_size)
{
    ...
    volatile struct queue_head *head = (queue_head*)memory;
    if (shm_exist == false) {
...
static int write_file(queue_t *queue, void *data, uint size)
{
    volatile struct queue_head *head = (queue_head*)queue->memory;
...
static void putmem(queue_t *queue, uint *pos_tail, byte *data, uint size)
{
    volatile struct queue_head *head = (queue_head*)queue->memory;
    byte *buf = queue->memory + sizeof(struct queue_head);
    uint tail_left = head->mem_size - *pos_tail;
...
int queue_push(queue_t *queue, void *data, uint size)
{
    ...
    uint pos_tail = head->pos_tail;
    putmem(queue, &pos_tail, (byte*)&size, sizeof(size));
    putmem(queue, &pos_tail, (byte*)data, size);
...
static byte* alloc_read_buf(queue_t *queue, uint size)
{
    if (queue->read_buf == NULL || queue->read_buf_size < size) {
        byte  *buf = queue->read_buf;
...
static int read_file(queue_t *queue, void **data, uint *size)
{
    volatile struct queue_head *head = (queue_head*)queue->memory;
...
static void getmem(queue_t *queue, uint *pos_head, byte *data, uint size)
{
    volatile struct queue_head *head = (queue_head*)queue->memory;
    byte* buf = queue->memory + sizeof(struct queue_head);
    uint tail_left = head->mem_size - *pos_head;
...
int queue_pop(queue_t *queue, void **data, uint *size)
{
    ...
    uint msg_size = 0;
    uint pos_head = head->pos_head;
    if (check_mem(queue, sizeof(msg_size)) < 0)
        return -1;
    getmem(queue, &pos_head, (byte*)&msg_size, sizeof(msg_size));
    ...
    *size = msg_size;
    ...
    if (check_mem(queue, (sizeof(msg_size) + msg_size)) < 0)
        return -1;
    getmem(queue, &pos_head, (byte*)*data, msg_size);
